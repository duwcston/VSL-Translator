...
class SignLanguageDetector:
    ...
    def detect_from_image(
        self, image: np.ndarray, input_size: int = 640
    ...
    def _draw_detections(
        self, image: np.ndarray, detections: List[Dict], fps: Optional[float] = None
    ) -> np.ndarray:
        ...
        for det in detections:
            ...
            cv2.rectangle(
                annotated_image, (x1, y1 - 20), (x1 + text_size[0], y1), (0, 255, 0), -1
            )
            cv2.putText(
                annotated_image,
                label,
    ...
    def process_frame_for_websocket(
        ...
        if return_image:
            _, buffer = cv2.imencode(
                ".jpg", annotated_image, [cv2.IMWRITE_JPEG_QUALITY, 80]
            )
